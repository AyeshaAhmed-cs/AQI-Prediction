...
def fetch_live_data():
    ...
    if response.get('status') != 'ok':
        print(f"❌ API Error: {response.get('data', 'Unknown error')}")
        return pd.DataFrame()

    data = response['data']
    iaqi = data.get('iaqi', {}) # Safety catch for iaqi
...
if __name__ == "__main__":
    ...
    if features.empty:
        print("⚠️ No new engineered features to upload. Skipping insert.")
    else:
        # Fast upload settings to prevent the 'RemoteDisconnected' error
        fg.insert(features, write_options={"wait_for_job": False, "start_offline_materialization": False})

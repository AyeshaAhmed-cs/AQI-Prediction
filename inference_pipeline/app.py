...
# --- 5. REAL-TIME PREDICTION ENGINE (SMOOTHED & ROUNDED) ---
def predict_aqi(temp, hum, wind, pres, hour_offset=0):
    p_time = datetime.now() + timedelta(hours=hour_offset)
    
    decay = 0.95 if hour_offset > 0 else 1.0

    input_data = {
        "pm25": float(historical_data['pm25'].iloc[-1] * (1 + (temp - 25)/100)), 
        "pm10": float(historical_data['pm10'].iloc[-1]),
        "no2": float(historical_data['no2'].iloc[-1]),
        "so2": float(historical_data['so2'].iloc[-1]),
        "co": float(historical_data['co'].iloc[-1]),
        "o3": float(historical_data['o3'].iloc[-1]),
    ...
    # Return as an integer for standard AQI reporting
    return float(raw_pred)

# 1. Generate full 72-hour list for calculations
future_preds = []
for i in range(72):
    ...
    f_pres = weather_data['pres'] # Assuming pressure stays stable
    future_preds.append(predict_aqi(f_temp, f_hum, f_wind, f_pres, i))
...
# 1. Sunrise/Sunset with dynamic styling
...
# 2. Interactive AQI Scale
st.sidebar.markdown("### 📊 AQI REFERENCE")
# Calculate position on scale (0-300+)
...
# 3. Model Performance Expander (Forced visibility)
with st.sidebar.expander("🛠️ Model Performance Details"):
    # Pull dynamic metrics from registry

...
def predict_aqi(temp, hum, wind, pres, hour_offset=0):
    # 1. Get the correct future time
    p_time = (datetime.now() + timedelta(hours=5)) + timedelta(hours=hour_offset)
    
    # 2. Get the latest REAL data from Hopsworks (Don't simulate math on them!)
    ...
    input_data = {
        ...
        "co": float(latest['co']),
        "o3": float(latest['o3']),
        "temperature": float(temp),  # The weather API drives the change
        "humidity": float(hum),      # The weather API drives the change
        "wind_speed": float(wind),   # The weather API drives the change
        "pressure": float(pres),

...
# --- 5. GENUINE REAL-TIME PREDICTION ENGINE ---
def predict_aqi(temp, hum, wind, pres, hour_offset=0):
    # 1. Get the exact future timestamp in Karachi
    p_time = (datetime.now() + timedelta(hours=5)) + timedelta(hours=hour_offset)
    
    # 2. Get the absolute latest REAL pollutant data from Hopsworks
    # This is the "Ground Truth" the model starts from
    latest = historical_data.iloc[-1]

    # 3. Construct input exactly as the Model Registry expects
    input_data = {
        ...
        "co": float(latest['co']),
        "o3": float(latest['o3']),
        "temperature": float(temp),  # DYNAMIC: From Open-Meteo Forecast
        "humidity": float(hum),      # DYNAMIC: From Open-Meteo Forecast
        "wind_speed": float(wind),   # DYNAMIC: From Open-Meteo Forecast
        "pressure": float(pres),
        "hour": int(p_time.hour),    # DYNAMIC: The specific hour of the day
        "day": int(p_time.day),
        "month": int(p_time.month),
        "day_of_week": int(p_time.weekday()),
        "is_weekend": int(1 if p_time.weekday() >= 5 else 0),
        "aqi_change": float(latest.get('aqi_change', 0)),
        "pm25_change_rate": float(latest.get('pm25_change_rate', 0)),
    ...
    return float(model.predict(df.values)[0]), df, feat_cols


# 4. Generate 72 hours of unique predictions
future_preds = []
...
for i in range(72):
    ...
    # UPDATE THIS LINE to capture the df and feat_cols from the first prediction (i=0)
    preds, current_df, current_feats = predict_aqi(f_temp, f_hum, f_wind, f_pres, hour_offset=i)
    
    # Store the very first prediction's data for SHAP analysis later
    if i == 0:
        df_for_shap = current_df
        feats_for_shap = current_feats
        
    future_preds.append(preds)

# 5. Aggregate for the cards
live_prediction = future_preds[0] 

today_card_val = future_preds[12]      # Prediction for 12 hours from now
tomorrow_card_val = future_preds[36]   # Prediction for 36 hours from now
next_day_card_val = future_preds[60]   # Prediction for 60 hours from now
...
# --- 11. SYSTEM AUTOMATION & SYNC STATUS ---
st.sidebar.markdown("---")
st.sidebar.markdown("### 🤖 SYSTEM AUTOMATION")

if not historical_data.empty:
    # 1. Convert to datetime and STRIP the timezone info to avoid the TypeError
    last_entry_time = pd.to_datetime(historical_data['timestamp'].iloc[-1]).tz_localize(None)
    
    # 2. Match your local time (UTC+5 for PKT)
    now_pkt = datetime.now() + timedelta(hours=5)
    
    # 3. Now subtraction works perfectly
    diff = now_pkt - last_entry_time
    minutes_ago = int(diff.total_seconds() / 60)

    if minutes_ago > 90:
        st.sidebar.error(f"⚠️ Stale Data: {last_entry_time.strftime('%I:%M %p')} ({minutes_ago}m ago)")
    else:
        st.sidebar.success(f"✅ Data Freshness: {last_entry_time.strftime('%I:%M %p')} (PKT)")
else:

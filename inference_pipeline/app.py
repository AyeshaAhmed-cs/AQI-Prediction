...
def predict_aqi(temp, hum, wind, pres, hour_offset=0):
    ...
    live_pm25 = float(base_pm25 * dispersion_factor * time_decay * (1 + (temp - 25)/50))
    
    input_data = {
        "pm25": live_pm25, 
        "pm10": float(base_pm10 * dispersion_factor * time_decay),
        "no2": float(historical_data['no2'].iloc[-1] * dispersion_factor),
        "so2": float(historical_data['so2'].iloc[-1] * dispersion_factor),
        "co": float(historical_data['co'].iloc[-1] * dispersion_factor),
        "o3": float(historical_data['o3'].iloc[-1] * (1 + (hum - 50)/150)),
        ...
        "aqi_change": float(historical_data['aqi'].diff().iloc[-1] if len(historical_data) > 1 else 0),
        "pm25_change_rate": float(historical_data['pm25'].diff().iloc[-1] if len(historical_data) > 1 else 0),
        
        # KEY FIX: The rolling average now includes the NEW dynamic pm25
        "rolling_avg_aqi_24h": float((historical_data['aqi'].tail(23).sum() + (live_pm25 * 1.2)) / 24),
        "rolling_avg_pm25_24h": float((historical_data['pm25'].tail(23).sum() + live_pm25) / 24)
    }

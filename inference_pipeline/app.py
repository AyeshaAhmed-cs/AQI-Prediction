...
if not historical_data.empty:
    ...
    last_entry_time = pd.to_datetime(historical_data['timestamp'].iloc[-1])
    
    now_pkt = datetime.now() + timedelta(hours=5)
    diff = now_pkt - last_entry_time
    minutes_ago = int(diff.total_seconds() / 60)

    # If data is older than 90 mins, show a warning instead of success
    if minutes_ago > 90:
        st.sidebar.error(f"⚠️ Stale Data: {last_entry_time.strftime('%I:%M %p')} ({minutes_ago}m ago)")
        st.sidebar.caption("Pipeline is running but data isn't reaching Hopsworks.")
    else:

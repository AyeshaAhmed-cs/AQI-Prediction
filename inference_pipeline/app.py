...
for i in range(72):
    # Extract unique forecast values for THIS specific hour
    f_temp = forecast_data['temperature_2m'][i]
    f_hum = forecast_data['relativehumidity_2m'][i]
    f_wind = forecast_data['windspeed_10m'][i]
    # Surface pressure doesn't fluctuate as wildly, using current
    f_pres = weather_data['pres'] 

    preds = predict_aqi(f_temp, f_hum, f_wind, f_pres, hour_offset=i)
...
with tab1:
    ...
    # 1. Clean data for correlation: drop non-numeric and constant columns
    nums = historical_data.select_dtypes(include=[np.number])
    # Remove columns where all values are the same (Variance = 0)
    nums_filtered = nums.loc[:, nums.nunique() > 1] 
    
    with ch:
        st.markdown("### Feature Correlation")
        if not nums_filtered.empty:
            # Calculate correlation and drop any remaining rows/cols with NaNs
            ...
            st.warning("Not enough variable data to generate a correlation heatmap.")
    ...
    with ci:
        ...
        try:
            ...
            # Use the feature columns assigned in Section 5 for the index
            feat_cols = ["pm25", "pm10", "no2", "so2", "co", "o3", "temperature", "humidity", 
                         "wind_speed", "pressure", "hour", "day", "month", "day_of_week", 
                         "is_weekend", "aqi_change", "pm25_change_rate", "rolling_avg_aqi_24h", "rolling_avg_pm25_24h"]
...
if not historical_data.empty:
    # The pipeline now sends PKT directly, so we just read it as is
    last_entry_time = pd.to_datetime(historical_data['timestamp'].iloc[-1])
    local_time = last_entry_time.strftime("%I:%M %p")

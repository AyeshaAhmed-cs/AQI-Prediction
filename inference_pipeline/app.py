...
def init_hopsworks():
    ...
    hist_df = fg.read(read_options={"use_hive": True}).sort_values("timestamp", ascending=True)
...
# --- 5. DYNAMIC REACTIVE PREDICTION ENGINE ---
def predict_aqi(temp, hum, wind, pres, hour_offset=0):
    ...
    dispersion = 20 / (wind + 5) 
    
    simulated_pm25 = last_pm25 * dispersion * (1 + (temp - 25)/50)
    ...
    input_data = {
        ...
        # MATH FIX: We multiply the change by 5 to "wake up" the model's sensitivity
        "aqi_change": float((simulated_pm25 - last_pm25) * 5),
        "pm25_change_rate": float((simulated_pm25 - last_pm25) * 5),
        
        # ROLLING AVG FIX: Use the full 24-hour mean from history + current simulation
        "rolling_avg_aqi_24h": float((historical_data['aqi'].tail(23).mean() + (last_aqi * dispersion)) / 2),

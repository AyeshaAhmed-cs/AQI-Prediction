...
def sync_clock():
    ...
    st.markdown(f'<div class="digital-clock">🕒 {now}</div>', unsafe_allow_html=True)
...
def predict_aqi(temp, hum, wind, pres, hour_offset=0):
    ...
    dispersion_factor = 10 / (wind + 1) 
    
    # Use the decay factor to simulate pollutants settling or dispersing over time
    time_decay = 0.98 ** (hour_offset / 24) 

    # Fetch baseline from historical data
    base_pm25 = float(historical_data['pm25'].iloc[-1])
    base_pm10 = float(historical_data['pm10'].iloc[-1])
    base_no2 = float(historical_data['no2'].iloc[-1])

    input_data = {
        # Dynamically scale pollutants based on weather and time decay
        "pm25": float(base_pm25 * dispersion_factor * time_decay * (1 + (temp - 25)/100)), 
        "pm10": float(base_pm10 * dispersion_factor * time_decay),
        "no2": float(base_no2 * dispersion_factor),
        "so2": float(historical_data['so2'].iloc[-1] * dispersion_factor),
        "co": float(historical_data['co'].iloc[-1] * dispersion_factor),
        "o3": float(historical_data['o3'].iloc[-1] * (1 + (hum - 50)/200)), # Ozone often correlates with humidity/sun
        ...
        # Calculate realistic change rates instead of 0.0
        "aqi_change": float(historical_data['aqi'].diff().iloc[-1] * time_decay),
        "pm25_change_rate": float(historical_data['pm25'].diff().iloc[-1] * time_decay),
        "rolling_avg_aqi_24h": float(historical_data['aqi'].tail(24).mean()),
        "rolling_avg_pm25_24h": float(historical_data['pm25'].tail(24).mean())
    }
    ...
    # Use model to predict based on these dynamic inputs
...
# UPDATED: Use the new _card_val variables here
...
for col, val, label in zip([c1, c2, c3], card_values, card_labels):
    ...
    col.markdown(f"""
        <div style="text-align:center; padding:20px; border:1px solid #00F2FF33; border-radius:15px; background:rgba(0,242,255,0.02)">
            <p style="color:#888; margin-bottom:5px;">{label}</p>
            <h1 style="color:#00F2FF; margin:0;">{rounded_val}</h1>
        </div>
    """, unsafe_allow_html=True)

# --- 9. ANALYTICS ROW: GAUGE & TRAJECTORY ---
# 1. Generate standard timestamps for the X-axis
forecast_dates = [datetime.now() + timedelta(hours=i) for i in range(72)]

# 2. SMOOTHING LOGIC: Convert 72 jagged points into a smooth curve
x_raw = np.arange(len(future_preds))
x_smooth = np.linspace(x_raw.min(), x_raw.max(), 300) # Increase density
spl = make_interp_spline(x_raw, future_preds, k=3)    # Cubic Spline
y_smooth = spl(x_smooth)
smooth_dates = [datetime.now() + timedelta(hours=float(h)) for h in x_smooth]

# 3. Create Columns for "Face-to-Face" Alignment
...
with col_gauge:
    ...
    fig_gauge = go.Figure(go.Indicator(
        mode = "gauge+number",
        value = live_prediction, # This shows your current 90/91 AQI
        domain = {'x': [0, 1], 'y': [0, 1]},
        gauge = {
    ...
    fig_gauge.update_layout(
        ...
        font={'color': "white", 'family': "Orbitron"}, height=400, # Matches graph height
...
with col_graph:
    ...
    # Using the smooth_dates and y_smooth for a professional look
    fig_traj = px.line(
        x=smooth_dates, 
        y=y_smooth,
    ...
    fig_traj.update_layout(
        paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
        height=400, # Matches gauge height
...
sun_icon = "☀️" if weather_data['is_day'] else "🌙"
day_status = "DAYTIME" if weather_data['is_day'] else "NIGHTTIME"
status_color = "#00F2FF" if weather_data['is_day'] else "#8F3F97"
glow_shadow = "0 0 10px #00F2FF" if weather_data['is_day'] else "0 0 10px #8F3F97"
